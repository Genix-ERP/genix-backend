-- 390_re_apply_journal_enrichment_correct_logic.sql
--
-- Re-apply migration 385's correct logic.
--
-- Why this exists: a renumber operation (357 -> 389) inadvertently
-- re-ran the OLD vendor-fallback function definition on top of 385's
-- correct one. Function now references procurement_contracts.vendor_id
-- which doesn't satisfy fk_jel_contract (FK targets contracts.id),
-- breaking CreateBillFromPO with: TT §4.5: account 6010 ... requires
-- contract (shartnoma).
--
-- Fix: copy 385's body verbatim into a new migration so DBs that
-- already recorded 389 will run THIS one and restore the correct
-- function.
--
-- Idempotent: pure CREATE OR REPLACE FUNCTION.

-- Original 385 header below (kept for context):
