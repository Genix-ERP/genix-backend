-- Migration 348: Enrich journal_entry_lines.contract_id with a fallback path
-- when the source document doesn't carry a contract_id directly.
--
-- Problem: TT §4.5 requires accounts 4010/6010 lines to carry contract_id.
-- The migration 325 enrichment trigger only reads contract_id from the
-- source document (sales_invoice / purchase_invoice). If the user creates
-- a vendor bill without explicitly attaching a contract — common when the
-- bill is auto-generated from an intercompany PO or a one-off purchase —
-- contract_id stays NULL and the migration 326 enforcement trigger
-- rejects the journal entry with:
--    "TT §4.5: account 6010 ... requires contract (shartnoma)"
--
-- This migration adds a fallback chain after the existing source-document
-- lookup: when contract_id is still null, try the most recent active
-- procurement contract for the vendor (purchase) or sales contract for
-- the customer (sale). If one exists, the journal entry passes; if not,
-- the user gets the same error as before so the audit trail is still
-- enforced when there's genuinely no contract on file.
...
BEGIN
    ...
    -- contact_id enrichment (unchanged from migration 325)
    ...
    -- warehouse_id enrichment (unchanged from migration 325)
    ...
    -- employee_id enrichment (unchanged from migration 325)
    ...
    -- contract_id enrichment with vendor/customer fallback
    IF NEW.contract_id IS NULL AND v_analytics_types ? 'shartnoma' THEN
        ...
        -- Fallback: when the source document has no contract_id, look up
        -- the most recent active procurement_contract for the contact
        -- (vendor on purchase, customer on sale). This unblocks bills
        -- where the user has a contract on file with the supplier but
        -- didn't attach it explicitly to the bill.
...
COMMENT ON FUNCTION fn_auto_enrich_journal_line_analytics() IS
    'TT §4.5: auto-fill analytics on journal_entry_lines. Falls back to '
    'the contact''s most recent active procurement_contract when the '
    'source document carries no contract_id. (Updated in migration 348.)';
