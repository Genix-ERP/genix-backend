...
--
-- RENUMBERED 444 -> 472 when ported from the yuksalish branch. On that branch
-- this was 444; on main, 444 is already 444_expenses_v2.sql. The migration
-- runner keys applied migrations by the integer version parsed from the
-- filename, so shipping this as 444 would have been SILENTLY SKIPPED on every
-- database that had already applied expenses_v2 — the tables would never be
-- created and all five endpoints would 500 at runtime with no migration error
-- to point at. Do not renumber it back.
