...
func (h *Handler) ListBOMs(c *gin.Context) {
	...
	baseQuery := `
		SELECT b.id, b.code, b.name, b.product_id, b.bom_type, b.quantity, b.version,
			   ...
			   + COALESCE((SELECT SUM((COALESCE(bo.setup_time_minutes, 0) + COALESCE(bo.run_time_minutes, 0)) / 60.0 * COALESCE(wc.hourly_cost, 0) / GREATEST(COALESCE(wc.capacity_per_hour, 1), 1))
...
func (h *Handler) RunReplenishment(c *gin.Context) {
	...
	for vendorID, items := range vendorItems {
		...
		`, poID, tenantID, organizationID, orderNumber, vid, time.Now(), "draft", "unpaid",
	...
	if len(noVendorItems) > 0 {
		...
		`, poID, tenantID, organizationID, orderNumber, time.Now(), "draft", "unpaid",
